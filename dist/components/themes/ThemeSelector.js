...
export const ThemeSelector = memo(({ showLabel = true, ...props }) => {
    const dispatch = useDispatch();
    const { current, list } = useSelector((state) => state.ui.themes);
    return (_jsxs(Container, { alignContent: Align.Center, grow: false, children: [showLabel && _jsx(InputLabel, { children: "Theme" }, void 0), _jsx(DropdownInput, { defaultValue: current, name: "theme", menu: list.map((i) => {
                    return {
                        label: i.name,
                        value: i.id,
                    };
